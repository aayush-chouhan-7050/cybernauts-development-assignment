...
export const createUser = async (userData: Omit<IUser, '_id' | 'createdAt' | 'friends'>): Promise<IUser> => {
...
export const linkUsers = async (userId: string, friendId: string): Promise<void> => {
  if (userId === friendId) {
...
export const unlinkUsers = async (userId: string, friendId: string): Promise<void> => {
    const user = await User.findById(userId);
    const friend = await User.findById(friendId);
...
export const deleteUser = async (id: string): Promise<IUser | null> => {
  ...
  if (user.friends.length > 0) {
    throw new ConflictError('User cannot be deleted while they have friends. Please unlink them first.');
  }
...
export const getGraphData = async () => {
    ...
    const userMap = new Map<string, IUser>(users.map(user => [user._id, user]));
